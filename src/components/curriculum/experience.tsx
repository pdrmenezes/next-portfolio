...
export function CurriculumExperience({ job }: { job: (typeof CURRICULUM_DATA)[number] }) {
  return (
    <details className={`group px-2`}>
      <summary
        className={`group border-b-whiteish border-t-whiteish flex w-full cursor-pointer items-center gap-3 border-t px-2 py-4 text-sm sm:justify-between sm:gap-5 sm:px-10 sm:py-5 sm:text-base`}
      >
        ...
        <div className="flex flex-1 items-center justify-between gap-1">
          ...
          <h4 className="sm:text-whiteish text-xs text-neutral-400 transition-transform duration-500 sm:text-base sm:group-hover:translate-x-2">
            {job.time}
          </h4>
        </div>
      </summary>
      <div className="my-6" id="project-body">
        <div className="my-3 px-4 leading-loose text-pretty md:leading-relaxed">
          <div className="my-5 flex flex-wrap gap-2">
            {job.mainSkills.map((skill) => (
              <p className="bg-whiteish text-blackish w-fit rounded-full px-3 py-0.5 text-xs font-medium whitespace-nowrap" key={skill}>
                {skill}
              </p>
            ))}
          </div>
